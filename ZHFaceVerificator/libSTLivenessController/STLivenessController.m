...
- (instancetype)initWithApiKey:(NSString *)apiKeyStr
    ...
    if (self) {
        if (!detectionArr) {
            ...
            // 资源路径
            _bundlePathStr = st_liveness_resourcePath;
            // 模型路径
            ...
            NSString *financeLicensePathStr =
                [FrameworkBundle pathForResource:@"SenseID_OCR" ofType:@"lic"];
...
-(void)setEachStepTimeOut:(NSTimeInterval)eachStepTimeOut{
    _eachStepTimeOut = eachStepTimeOut;
    if (eachStepTimeOut>0) {
        if (_detector) {
            [_detector setTimeOutDuration:eachStepTimeOut];
        }
    }
}
