...
- (instancetype)initWithCount:(NSInteger)count margin:(CGFloat)margin completeBlock:(void(^)(NSString *text))complete
{
    if (self = [super init]) {
...
- (void)configTextField
{
    ...
    for (NSInteger i = 0; i < self.itemCount; i++)
    {
        ...
        label.textColor = UIColorFromRGB(0x6396ea);;
...
- (void)tfEditingChanged:(UITextField *)textField
{
    ...
    for (int i = 0; i < self.itemCount; i++)
    {
        ...
        if (i < textField.text.length) {
            ...
            line.backgroundColor = UIColorFromRGB(0x6396ea);
        } else {
            label.text = nil;
            line.backgroundColor = UIColorFromRGB(0xbbbbbb);
        }
